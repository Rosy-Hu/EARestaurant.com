...
function openModal({ kicker, title, subtitle, body, meta = [] }) {
...
function renderStatic() {
  $("#ratingStars").textContent = stars(5);
  $("#reviewCountButton").textContent = `${data.restaurant.reviewCount}条 ›`;
  $("#pricePerson").textContent = data.restaurant.perCapita;
  $("#photoCount").textContent = "3";
  $("#birthdayNames").textContent = data.birthdays.map((person) => person.name).join("、");
...
function bindEvents() {
  ...
  $("#birthdayButton")?.addEventListener("click", () => toast("已把祝福送到九月生日宴"));
